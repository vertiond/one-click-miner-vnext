...
CD ..
DEL build\verthash-ocm.exe >nul 2>&1
wails build
ECHO "Sign the release assembly now on the windows machine if desired, then:"
PAUSE
CD build 
7z -sdel -aou a verthash-ocm-%ver%-windows-x64.zip vertcoin-ocm.exe
CD ..
wails build -d
ECHO "Sign the debug assembly now on the windows machine if desired, then:"
PAUSE
CD build
7z -sdel -aou a verthash-ocm-%ver%-windows-x64-debug.zip vertcoin-ocm.exe
CD ..
